...
const Requests = () => {
  ...
  const handleAccept = async (code) => {
    try {
      await axios.post("http://localhost:4500/api/requests/accept", { code });
      setRequests(requests.filter((request) => request.code !== code)); // Remove accepted request
    } catch (error) {
  ...
  const handleReject = async (code) => {
    try {
      await axios.post("http://localhost:4500/api/requests/reject", { code });
